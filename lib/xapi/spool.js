// lib/xapi/spool.js
// ─────────────────────────────────────────────────────────────
// xAPI statement 로컬 수집 · 집계 엔진 (Phase 1 — statement_id idempotency).
...
// xAPI Statement는 UUID v4 'id' 필드를 가짐 (idempotency dedup용).
//
// ─────────────────────────────────────────────────────────────
// 이중 트랙 정리 (Phase 2 문서화):
//   1) learning_logs (db/learning-log-helper.js):
//      - 다채움 내부 LRS 대시보드·포트폴리오 자동 등록·성장 목표 진행률 등
//        다채움 자체 운영을 위한 트랙.
//      - statement_json 은 단순 xAPI 형식이지만 verb URL 은 adlnet.gov 표준을 사용
//        (AIDT 표준 URL 과는 다름). 즉 다채움 내부 용도 전용.
//
//   2) xapi_statement_spool (본 모듈):
//      - AIDT KERIS 허브 송신용 트랙. verb·activity-type·extension URL 이 모두
//        aidtbook.kr 표준이며, JSON Schema(DATA_API_007) 통과 보장 대상.
//      - Phase 3 에서 aidt-transmitter 가 drainUnsent() → POST → markSent() 사이클로 전송.
//
//   동일 활동이 두 테이블에 모두 기록되어도 OK (이중 트랙 유지). 각 트랙의
//   소비자가 명확히 분리되므로 데이터 충돌이 없다.
//
//   Phase 3 에서는 spool 만 KERIS 로 송신하며, learning_logs 는 다채움 내부에만 보관.
//   양 트랙의 분리는 송신 어댑터 단계에서 자연스럽게 이루어진다.
// ─────────────────────────────────────────────────────────────
const db = require('../../db');
const { userUuid, newStatementId } = require('./common');

const insSpool = db.prepare(`
  INSERT INTO xapi_statement_spool
    (statement_id, user_uuid, user_id, area, verb, statement_json, event_timestamp,
...
// statement_id 기준 dedup 체크 (재전송·재시도 시 동일 statement 중복 INSERT 방지)
const selStatementById = db.prepare(
  'SELECT id FROM xapi_statement_spool WHERE statement_id = ? LIMIT 1'
);
 ...
 * @returns {{ spoolId:number|null, statementId:string|null, statsUpdated:number, dedup?:boolean, error?:string }}
 */
function enqueue(builderResult, ctx = {}) {
  const res = { spoolId: null, statementId: null, statsUpdated: 0 };
  try {
    ...
    // xAPI Statement id (UUID v4) 보장. builder 가 makeStatement 통해 부여하지만,
    // 직접 statement 를 만든 경우 누락 가능 — 여기서 안전망.
    if (!statement.id) {
      statement.id = newStatementId();
    }

    // idempotency: statement_id 중복 INSERT 방지
    const dup = selStatementById.get(statement.id);
    if (dup) {
      res.spoolId = dup.id;
      res.statementId = statement.id;
      res.dedup = true;
      return res;
    }
...
function drainUnsent(limit = 500) {
  try {
    return db.prepare(
      `SELECT id, statement_id, user_uuid, area, verb, statement_json, event_timestamp
...
function record(area, builderOrResult, ctx, payload) {
  try {
    ...
    return { spoolId: null, statementId: null, statsUpdated: 0, error: String(e && e.message || e) };
  }
}
