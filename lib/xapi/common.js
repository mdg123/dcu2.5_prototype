...
const VERB = {
  ...
  // teaching 도메인의 gave (assignment의 gave와 URL이 다름 — 영역별 profile 분리)
  teachingGave:{ id: `${AIDT_DOMAIN}/xapi/profiles/teaching/1.0/verbs/gave`,                display: { 'ko-KR': '피드백함', 'en-US': 'gave'        }},
};

// ─────────────────────────────────────────────────────────────
// AIDT 표준 activity-type slug 매핑 헬퍼 (Phase 2)
// 다채움 콘텐츠 유형 → AIDT navigation 4분기 slug
...
function mapNavigationSlug(contentType) {
  ...
  // 영상·음성도 navigation 으로 들어오는 경우(media builder가 아닌 곳)에 대비:
  if (t === 'video' || t === 'audio' || t === 'v' || t === 'a') return 'etc-content';
  return 'etc-content';
}

// ─────────────────────────────────────────────────────────────
// 감정 이모티콘 → 리커트 정수(1~5) 변환 테이블 (Phase 2 — Survey 영역)
// PDF §1.4 emotion-today-survey-detail.response int 요구
// ─────────────────────────────────────────────────────────────
