 ...
 *
 * ⚠ 중첩 `questions` 도 함께 벗긴다 (2026-08-07 실측 누출):
 *   `GET /api/self-learn/map/nodes/:id?withFallbackProblems=1` 의 폴백 문항은
 *   `problems[].questions[].answer` 형태로 **한 겹 더 안쪽**에 정답을 담는다.
 *   최상위 키만 지우던 이전 구현은 이것을 통과시켰다 —
 *     problems[0].questions[0].answer="4" / explanation="정답은 ①이다 …"
 *   깊이는 4로 제한한다(꾸러미 안의 꾸러미까지. 순환 시드가 있어도 무한루프 없음).
 */
function _scrubQuestion(q, depth = 0) {
  if (!q || typeof q !== 'object') return q;
  const out = Array.isArray(q) ? q.slice() : { ...q };
  for (const f of ANSWER_FIELDS) delete out[f];
  if (depth < 4 && Array.isArray(out.questions)) {
    out.questions = out.questions.map((n) => _scrubQuestion(n, depth + 1));
  }
  return out;
}
...
function stripAnswers(questions, viewer, opts = {}) {
  ...
  // ⚠ map(_scrubQuestion) 로 넘기면 두 번째 인자(index)가 depth 로 들어간다 — 반드시 래핑.
  return questions.map((q) => _scrubQuestion(q));
}
...
function stripAnswersJson(rawJson, viewer, opts = {}) {
  ...
  return JSON.stringify(arr.map((q) => _scrubQuestion(q)));
}
