...
function drawActivity(doc, item, num, total, options) {
  ...
  // 메타 — activity_type을 한국어 라벨로 매핑
  const ACTIVITY_TYPE_LABEL = {
    extracurricular: '비교과 활동',
    homework: '과제', lesson: '수업', exam: '평가', contents: '콘텐츠',
    self_learn: '자기 학습', project: '프로젝트', volunteer: '봉사 활동',
    club: '동아리', award: '수상', activity: '활동'
  };
  const metaParts = [];
  ...
  if (item.activity_type) metaParts.push(ACTIVITY_TYPE_LABEL[item.activity_type] || item.activity_type);
  ...
  if (Array.isArray(tags) && tags.length > 0) {
    ...
    cursorY += 20;
    // 역량 태그 정규화 (공백 차이로 같은 태그가 다르게 보이는 문제 방지)
    const COMPETENCY_LABEL_MAP = {
      '자기관리': '자기관리', '지식정보처리': '지식정보처리',
      '창의적사고': '창의적 사고', '창의적 사고': '창의적 사고',
      '심미적감성': '심미적 감성', '심미적 감성': '심미적 감성',
      '협력적소통': '협력적 소통', '협력적 소통': '협력적 소통',
      '공동체': '공동체'
    };
    const normalizeCompetency = (t) => {
      const s = String(t || '').trim();
      return COMPETENCY_LABEL_MAP[s] || COMPETENCY_LABEL_MAP[s.replace(/\s+/g, '')] || s;
    };
    let chipX = 50;
    const chipY = cursorY;
    tags.forEach(tag => {
      const txt = `#${safe(normalizeCompetency(tag))}`;
