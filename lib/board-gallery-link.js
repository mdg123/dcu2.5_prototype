...
function extractPostMedia(post) {
  ...
  const push = (rawUrl, hint) => {
    ...
    // base64 본문 이미지(data:image/png;base64,…)는 **첨부로 받지 않는다**.
    //   gallery_attachments.url·student_gallery.image_url 은 1000자로 잘라 저장하므로
    //   base64 를 넣으면 잘린 쓰레기 URL = 깨진 이미지가 된다(빈 이미지보다 더 나쁨).
    //   정상 경로에서는 routes/board.js 가 **저장 전에** lib/inline-data-media.js 로
    //   파일로 바꿔 주므로 여기까지 data: 가 올 일이 없다. 이 줄은 그 관문을 우회한
    //   경로(레거시 데이터·직접 INSERT)에 대한 방어다.
